...
const storeNavigation: ReadonlyArray<NavigationItem> = [
  ...
  { href: "/admin/email-settings", label: "Configuración de correos", icon: Mail },
  { href: "/admin/email-logs", label: "Logs de correos", icon: FileText },
  { href: "/admin/restock-alerts", label: "Alertas de restock", icon: Bell },
  { href: "/admin/abandoned-carts", label: "Carritos abandonados", icon: ShoppingCart },
  { href: "/admin/referrals", label: "Referidos", icon: UserPlus },
  { href: "/admin/subscriptions", label: "Suscripciones", icon: Globe },
] as const;

const integrationsNavigation: ReadonlyArray<NavigationItem> = [
  { href: "/admin/webhook-config", label: "Configuración de webhooks", icon: Webhook },
...
const navigationSections: ReadonlyArray<{
  ...
  { id: "general", label: "General", items: primaryNavigation },
...
function NavigationLink(
  ...
  const linkClasses = cx(
    props.compact
      ...
      : "group flex min-h-[2.125rem] items-center gap-2.5 rounded-lg px-2.5 py-1.5 text-left transition-[background-color,color] duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-brand focus-visible:ring-offset-2 focus-visible:ring-offset-surface-canvas",
    isActive
      ? props.compact
        ...
        >
          {isCollapsed ? (
            <motion.div
              ...
              className="flex w-full flex-col items-center gap-1 rounded-xl border border-border-soft bg-surface-canvas px-2 py-3 max-h-[calc(100vh-3rem)] overflow-y-auto overflow-x-hidden"
            >
              <button
                type="button"
    ...
    <div className="flex h-full w-full flex-col border-r border-border-soft bg-surface-canvas p-3">
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-[11px] text-text-muted capitalize">{props.userRole}</p>
        </div>
