...
export function LiveSearch() {
  ...
  const options = useMemo<SearchOption[]>(() => {
    const nextOptions: SearchOption[] = [
      ...results.categories.map((category) => ({ type: "category" as const, href: category.href })),
      ...results.products.map((product) => ({ type: "product" as const, href: product.href })),
    ];
    ...
    return nextOptions;
  }, [results.categories, results.products, searchHref, trimmedQuery.length]);
        ...
        <AnimatePresence>
          {isDesktopOpen && shouldRenderDropdown ? (
            <LiveSearchDropdown
              ...
              onViewAllHover={() => setActiveIndex(results.categories.length + results.products.length)}
          ...
          {isMobileOpen ? (
            ...
            >
              <motion.button
                ...
                onClick={closeMobile}
                className="absolute inset-0 bg-ink-900/10 backdrop-blur-sm"
                initial={reduceMotion ? { opacity: 1 } : { opacity: 0 }}
                animate={reduceMotion ? { opacity: 1 } : { opacity: 1 }}
                exit={reduceMotion ? { opacity: 1 } : { opacity: 0 }}
                transition={reduceMotion ? { duration: 0 } : overlayTransition}
