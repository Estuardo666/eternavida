...
export async function PUT(request: NextRequest, context: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try {
    ...
    if (!existing) {
      return createErrorResponse(404, "NOT_FOUND", "Pickup location not found.", timestamp);
    }
    ...
    const message = error instanceof Error ? error.message : "Failed to update pickup location.";
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Failed to delete pickup location.";
    const status = message.includes("not found") ? 404 : 500;
    const code = status === 404 ? "NOT_FOUND" : "INTERNAL_ERROR";
