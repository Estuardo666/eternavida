...
import { deleteProductBadgePreset, updateProductBadgePreset } from "@/services/admin-catalog/badge-preset-crud";
import { mapAdminProductBadgePresetItem } from "@/services/admin-catalog/get-catalog-admin-data";
...
export async function PUT(request: NextRequest, context: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try {
    ...
    const validationResult = adminProductBadgePresetFormSchema.safeParse(body);
    ...
    const badgePreset = await updateProductBadgePreset(id, validationResult.data);

    return NextResponse.json(
      {
        success: true,
        data: {
          badgePreset: mapAdminProductBadgePresetItem(badgePreset),
        },
        timestamp,
      },
      { status: 200 },
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
      return createErrorResponse(409, "CONFLICT", "Badge preset label already exists.", timestamp);
    }
    ...
    const message = error instanceof Error ? error.message : "Failed to update badge preset.";
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try {
    ...
    const { id } = await context.params;
    const result = await deleteProductBadgePreset(id);
    ...
    const message = error instanceof Error ? error.message : "Failed to delete badge preset.";
